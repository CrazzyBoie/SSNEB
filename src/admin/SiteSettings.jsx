...
import { FaSave, FaUndo, FaImage, FaPalette } from 'react-icons/fa';
...
const SiteSettings = () => {
  const [settings, setSettings] = useLocalStorage('admin_site_settings', defaultSiteSettings);
  ...
  const handleColorChange = (colorVar, value) => {
    setFormData(prev => ({ ...prev, [colorVar]: value }));
    document.documentElement.style.setProperty(
      colorVar === 'primaryColor' ? '--color-primary' : 
      colorVar === 'secondaryColor' ? '--color-secondary' : '--color-accent', 
      value
    );
  };
  ...
  const handleReset = () => {
    if (window.confirm('Reset all settings to defaults?')) {
      ...
      document.documentElement.style.setProperty('--color-primary', defaultSiteSettings.primaryColor);
      document.documentElement.style.setProperty('--color-secondary', defaultSiteSettings.secondaryColor);
      document.documentElement.style.setProperty('--color-accent', defaultSiteSettings.accentColor);
    }
  };
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <h2 style={{ fontSize: '1.8rem', color: 'var(--color-secondary)' }}>Site Settings</h2>
        <div style={{ display: 'flex', gap: '10px' }}>
          <button onClick={handleReset}
            style={{ padding: '10px 20px', background: '#e5e7eb', color: 'var(--color-dark)', border: 'none', borderRadius: 'var(--radius-sm)', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontWeight: 600 }}>
            <FaUndo /> Reset
          </button>
          <button onClick={handleSave}
            style={{ padding: '10px 20px', background: 'var(--color-success)', color: 'white', border: 'none', borderRadius: 'var(--radius-sm)', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontWeight: 600 }}>
            <FaSave /> Save Changes
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
        {/* Branding */}
        <div style={{ background: 'white', borderRadius: 'var(--radius-md)', padding: '28px', boxShadow: 'var(--shadow-card)' }}>
          <h3 style={{ marginBottom: '20px', color: 'var(--color-secondary)', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <FaImage /> Branding
          </h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div>
              <label style={{ display: 'block', marginBottom: '6px', fontWeight: 600, fontSize: '0.9rem' }}>School Tagline</label>
            ...
            <div>
              <label style={{ display: 'block', marginBottom: '6px', fontWeight: 600, fontSize: '0.9rem' }}>School Logo</label>
              <div style={{ border: '2px dashed #e5e7eb', borderRadius: 'var(--radius-md)', padding: '20px', textAlign: 'center', cursor: 'pointer' }}>
                ...
                <label htmlFor="logo-upload" style={{ cursor: 'pointer' }}>
                  {formData.logo ? (
                    <img src={formData.logo} alt="Logo" style={{ maxHeight: '100px', borderRadius: '50%' }} />
                  ) : (
                    <img src={LOGO_BASE64} alt="Default Logo" style={{ maxHeight: '100px', borderRadius: '50%' }} />
                  )}
                  <p style={{ fontSize: '0.85rem', color: 'var(--color-muted)', marginTop: '8px' }}>Click to change logo</p>
                </label>
              </div>
            </div>
          </div>
        ...
        <div style={{ background: 'white', borderRadius: 'var(--radius-md)', padding: '28px', boxShadow: 'var(--shadow-card)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div>
              <label style={{ display: 'block', marginBottom: '6px', fontWeight: 600, fontSize: '0.9rem' }}>Primary Color (Red)</label>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <input type="color" value={formData.primaryColor} onChange={e => handleColorChange('primaryColor', e.target.value)}
                  style={{ width: '50px', height: '40px', border: 'none', borderRadius: 'var(--radius-sm)', cursor: 'pointer' }} />
                <input type="text" value={formData.primaryColor} onChange={e => handleColorChange('primaryColor', e.target.value)}
                  style={{ flex: 1, padding: '10px', border: '2px solid #e5e7eb', borderRadius: 'var(--radius-sm)' }} />
              </div>
            </div>
            <div>
              <label style={{ display: 'block', marginBottom: '6px', fontWeight: 600, fontSize: '0.9rem' }}>Secondary Color (Blue)</label>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <input type="color" value={formData.secondaryColor} onChange={e => handleColorChange('secondaryColor', e.target.value)}
                  style={{ width: '50px', height: '40px', border: 'none', borderRadius: 'var(--radius-sm)', cursor: 'pointer' }} />
                <input type="text" value={formData.secondaryColor} onChange={e => handleColorChange('secondaryColor', e.target.value)}
                  style={{ flex: 1, padding: '10px', border: '2px solid #e5e7eb', borderRadius: 'var(--radius-sm)' }} />
              </div>
            </div>
            <div>
              <label style={{ display: 'block', marginBottom: '6px', fontWeight: 600, fontSize: '0.9rem' }}>Accent Color (Yellow)</label>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <input type="color" value={formData.accentColor} onChange={e => handleColorChange('accentColor', e.target.value)}
                  style={{ width: '50px', height: '40px', border: 'none', borderRadius: 'var(--radius-sm)', cursor: 'pointer' }} />
                <input type="text" value={formData.accentColor} onChange={e => handleColorChange('accentColor', e.target.value)}
                  style={{ flex: 1, padding: '10px', border: '2px solid #e5e7eb', borderRadius: 'var(--radius-sm)' }} />
              </div>
            </div>
          </div>
        ...
        {/* Contact Info */}
        <div style={{ background: 'white', borderRadius: 'var(--radius-md)', padding: '28px', boxShadow: 'var(--shadow-card)' }}>
          <h3 style={{ marginBottom: '20px', color: 'var(--color-secondary)' }}>Contact Information</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div>
              <label style={{ display: 'block', marginBottom: '6px', fontWeight: 600, fontSize: '0.9rem' }}>Phone</label>
            ...
            <div>
              <label style={{ display: 'block', marginBottom: '6px', fontWeight: 600, fontSize: '0.9rem' }}>Address</label>
              <input type="text" name="address" value={formData.address} onChange={handleChange}
                style={{ width: '100%', padding: '10px', border: '2px solid #e5e7eb', borderRadius: 'var(--radius-sm)' }} />
            </div>
            <div>
              <label style={{ display: 'block', marginBottom: '6px', fontWeight: 600, fontSize: '0.9rem' }}>Office Hours</label>
              <input type="text" name="officeHours" value={formData.officeHours} onChange={handleChange}
                style={{ width: '100%', padding: '10px', border: '2px solid #e5e7eb', borderRadius: 'var(--radius-sm)' }} />
            </div>
          </div>
        </div>
        ...
        {/* Social Media */}
        <div style={{ background: 'white', borderRadius: 'var(--radius-md)', padding: '28px', boxShadow: 'var(--shadow-card)' }}>
          <h3 style={{ marginBottom: '20px', color: 'var(--color-secondary)' }}>Social Media Links</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div>
              <label style={{ display: 'block', marginBottom: '6px', fontWeight: 600, fontSize: '0.9rem' }}>Facebook</label>
              <input type="url" name="facebook" value={formData.facebook} onChange={handleChange}
                style={{ width: '100%', padding: '10px', border: '2px solid #e5e7eb', borderRadius: 'var(--radius-sm)' }} />
            </div>
            <div>
              <label style={{ display: 'block', marginBottom: '6px', fontWeight: 600, fontSize: '0.9rem' }}>YouTube</label>
              <input type="url" name="youtube" value={formData.youtube} onChange={handleChange}
                style={{ width: '100%', padding: '10px', border: '2px solid #e5e7eb', borderRadius: 'var(--radius-sm)' }} />
            </div>
            <div>
              <label style={{ display: 'block', marginBottom: '6px', fontWeight: 600, fontSize: '0.9rem' }}>Instagram</label>
              <input type="url" name="instagram" value={formData.instagram} onChange={handleChange}
                style={{ width: '100%', padding: '10px', border: '2px solid #e5e7eb', borderRadius: 'var(--radius-sm)' }} />
            </div>
            <div>
              <label style={{ display: 'block', marginBottom: '6px', fontWeight: 600, fontSize: '0.9rem' }}>TikTok</label>
