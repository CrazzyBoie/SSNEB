import React, { useState, useEffect, useRef } from 'react';
import { Link, useLocation, useNavigate } from 'react-router-dom';
import { useLanguage } from '../context/LanguageContext';
import { useAuth } from '../context/AuthContext';
import {
  FaBars, FaTimes, FaPhone, FaEnvelope, FaFacebook, FaTiktok,
  FaUserCircle, FaSignOutAlt, FaUserShield, FaChalkboardTeacher,
  FaUserGraduate, FaSignInAlt, FaChevronDown
} from 'react-icons/fa';
import { useFirestore } from '../hooks/useFirestore';
import { defaultSiteSettings } from '../data/defaultData';

const LOGO_BASE64 = "data:image/jpeg;base64,";

const ROLE_DASHBOARD = {
  admin:   '/admin/dashboard',
  teacher: '/teacher/students',
  student: '/student/results',
};
const ROLE_ICON = {
  admin:   <FaUserShield size={12} />,
  teacher: <FaChalkboardTeacher size={12} />,
  student: <FaUserGraduate size={12} />,
};
const ROLE_COLOR  = { admin: '#C0392B', teacher: '#1A3A6B', student: '#059669' };
const ROLE_LABEL  = { admin: 'Administrator', teacher: 'Teacher', student: 'Student' };

const Navbar = () => {
  const { t, lang, toggleLanguage } = useLanguage();
  const { currentUser, logout } = useAuth();
  const location = useLocation();
  const navigate = useNavigate();
  const [scrolled, setScrolled]         = useState(false);
  const [mobileOpen, setMobileOpen]     = useState(false);
  const [userMenuOpen, setUserMenuOpen] = useState(false);
  const [siteSettings]                  = useFirestore('admin_site_settings', defaultSiteSettings);
  const userMenuRef = useRef(null);

  const logo      = siteSettings?.logo || LOGO_BASE64;
  const roleColor = currentUser ? ROLE_COLOR[currentUser.role] : '#1A3A6B';

  useEffect(() => {
    const onScroll = () => setScrolled(window.scrollY > 40);
    window.addEventListener('scroll', onScroll);
    return () => window.removeEventListener('scroll', onScroll);
  }, []);

  useEffect(() => { setMobileOpen(false); }, [location]);

  useEffect(() => {
    const handler = (e) => {
      if (userMenuRef.current && !userMenuRef.current.contains(e.target))
        setUserMenuOpen(false);
    };
    document.addEventListener('mousedown', handler);
    return () => document.removeEventListener('mousedown', handler);
  }, []);

  const handleLogout = () => { logout(); setUserMenuOpen(false); navigate('/'); };

  const navLinks = [
    { path: '/',            label: t('home')        },
    { path: '/about',       label: t('about')       },
    { path: '/academics',   label: t('academics')   },
    { path: '/admissions',  label: t('admissions')  },
    { path: '/faculty',     label: t('faculty')     },
    { path: '/gallery',     label: t('gallery')     },
    { path: '/news-events', label: t('news')        },
    { path: '/notice-board',label: t('notices')     },
    { path: '/contact',     label: t('contact')     },
  ];

  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap');

        :root {
          --navy: #0F2348;
          --navy-mid: #1A3A6B;
          --red: #C8102E;
          --red-light: #E8192C;
          --gold: #D4A017;
          --white: #FFFFFF;
          --off-white: #F8F9FC;
          --text-muted: #64748B;
          --border: rgba(15,35,72,0.08);
          --shadow-sm: 0 1px 3px rgba(0,0,0,0.06), 0 1px 2px rgba(0,0,0,0.04);
          --shadow-md: 0 4px 16px rgba(15,35,72,0.10), 0 1px 4px rgba(0,0,0,0.04);
          --shadow-lg: 0 12px 40px rgba(15,35,72,0.14), 0 4px 12px rgba(0,0,0,0.06);
          --ease: cubic-bezier(0.4,0,0.2,1);
          --font: 'Plus Jakarta Sans', sans-serif;
        }

        .ssn-navbar * { box-sizing: border-box; font-family: var(--font); }

        /* ── Top bar ── */
        .ssn-topbar {
          background: var(--navy);
          color: rgba(255,255,255,0.82);
          font-size: 0.78rem;
          font-weight: 500;
          letter-spacing: 0.01em;
          padding: 7px 0;
          border-bottom: 1px solid rgba(255,255,255,0.06);
        }
        .ssn-topbar-inner {
          max-width: 1280px;
          margin: 0 auto;
          padding: 0 24px;
          display: flex;
          justify-content: space-between;
          align-items: center;
        }
        .ssn-topbar-left { display: flex; gap: 20px; align-items: center; }
        .ssn-topbar-item { display: flex; align-items: center; gap: 6px; }
        .ssn-topbar-item svg { color: var(--gold); opacity: 0.9; }
        .ssn-topbar-right { display: flex; align-items: center; gap: 12px; }
        .ssn-topbar-label { font-size: 0.74rem; color: rgba(255,255,255,0.5); letter-spacing: 0.05em; text-transform: uppercase; }
        .ssn-social-link {
          width: 28px; height: 28px; border-radius: 6px;
          display: flex; align-items: center; justify-content: center;
          color: white; transition: all 0.2s var(--ease);
          text-decoration: none;
        }
        .ssn-social-link:hover { transform: translateY(-1px); opacity: 0.85; }
        .ssn-social-fb  { background: #1877F2; }
        .ssn-social-tik { background: #111; }

        /* ── Fixed wrapper ── */
        .ssn-navbar {
          position: fixed;
          top: 0; left: 0; right: 0;
          z-index: 1000;
        }
        .ssn-navbar-spacer {
          height: 107px; /* topbar ~35px + nav ~68px + 4px border */
        }

        /* ── Main nav ── */
        .ssn-nav {
          position: relative;
          z-index: 1;
          background: rgba(255,255,255,0.97);
          backdrop-filter: blur(12px);
          -webkit-backdrop-filter: blur(12px);
          border-bottom: 1px solid var(--border);
          transition: box-shadow 0.3s var(--ease);
        }
        .ssn-nav.scrolled {
          box-shadow: var(--shadow-md);
        }
        .ssn-nav-inner {
          max-width: 1280px;
          margin: 0 auto;
          padding: 0 24px;
          display: flex;
          align-items: center;
          justify-content: space-between;
          height: 68px;
          gap: 12px;
        }

        /* ── Logo ── */
        .ssn-logo {
          display: flex; align-items: center; gap: 12px;
          text-decoration: none; flex-shrink: 0;
        }
        .ssn-logo-img {
          width: 44px; height: 44px; border-radius: 50%;
          object-fit: cover; border: 2.5px solid var(--navy-mid);
          flex-shrink: 0;
          box-shadow: 0 0 0 3px rgba(26,58,107,0.08);
          transition: box-shadow 0.2s;
        }
        .ssn-logo:hover .ssn-logo-img { box-shadow: 0 0 0 4px rgba(26,58,107,0.15); }
        .ssn-logo-text { display: flex; flex-direction: column; line-height: 1.2; }
        .ssn-logo-name {
          font-size: 0.9rem; font-weight: 800; color: var(--navy);
          letter-spacing: -0.01em; white-space: nowrap;
        }
        .ssn-logo-sub {
          font-size: 0.7rem; font-weight: 600;
          color: var(--red); letter-spacing: 0.02em; white-space: nowrap;
        }

        /* ── Nav links ── */
        .ssn-links {
          display: flex; align-items: center; gap: 0;
          flex: 1; justify-content: flex-end;
          margin-right: 8px;
        }
        .ssn-link {
          position: relative;
          padding: 6px 11px;
          font-size: 0.815rem; font-weight: 600;
          color: var(--text-muted);
          text-decoration: none;
          letter-spacing: 0.01em;
          transition: color 0.18s var(--ease);
          white-space: nowrap;
          border-radius: 6px;
        }
        .ssn-link::after {
          content: '';
          position: absolute;
          bottom: -2px; left: 50%; right: 50%;
          height: 2.5px; border-radius: 2px;
          background: var(--red);
          transition: left 0.22s var(--ease), right 0.22s var(--ease);
        }
        .ssn-link:hover { color: var(--navy); }
        .ssn-link:hover::after { left: 11px; right: 11px; }
        .ssn-link.active { color: var(--navy); font-weight: 700; }
        .ssn-link.active::after { left: 11px; right: 11px; background: var(--red); }

        /* ── Right actions ── */
        .ssn-actions { display: flex; align-items: center; gap: 8px; flex-shrink: 0; }

        /* Lang toggle */
        .ssn-lang-btn {
          padding: 5px 12px; border-radius: 6px;
          border: 1.5px solid var(--border);
          background: var(--off-white);
          color: var(--navy); font-weight: 700;
          font-size: 0.75rem; cursor: pointer;
          letter-spacing: 0.04em;
          transition: all 0.18s var(--ease);
          font-family: var(--font);
        }
        .ssn-lang-btn:hover { border-color: var(--navy-mid); background: white; }

        /* Login btn */
        .ssn-login-btn {
          display: flex; align-items: center; gap: 6px;
          padding: 7px 16px; border-radius: 8px;
          background: white; border: 1.5px solid var(--navy-mid);
          color: var(--navy); font-weight: 700; font-size: 0.82rem;
          text-decoration: none; cursor: pointer;
          transition: all 0.18s var(--ease);
          font-family: var(--font);
          letter-spacing: 0.01em;
        }
        .ssn-login-btn:hover {
          background: var(--navy); color: white;
          transform: translateY(-1px); box-shadow: var(--shadow-sm);
        }

        /* Apply btn */
        .ssn-apply-btn {
          display: flex; align-items: center;
          padding: 8px 18px; border-radius: 8px;
          background: var(--red); color: white;
          font-weight: 700; font-size: 0.82rem;
          text-decoration: none; letter-spacing: 0.01em;
          transition: all 0.2s var(--ease);
          box-shadow: 0 2px 8px rgba(200,16,46,0.25);
          white-space: nowrap;
        }
        .ssn-apply-btn:hover {
          background: var(--red-light);
          transform: translateY(-1px);
          box-shadow: 0 4px 14px rgba(200,16,46,0.35);
        }

        /* ── User menu ── */
        .ssn-user-wrap { position: relative; }
        .ssn-user-btn {
          display: flex; align-items: center; gap: 8px;
          padding: 5px 12px 5px 6px; border-radius: 999px;
          border: 1.5px solid var(--border);
          background: white; cursor: pointer;
          transition: all 0.18s var(--ease);
          font-family: var(--font);
        }
        .ssn-user-btn:hover { border-color: rgba(26,58,107,0.2); box-shadow: var(--shadow-sm); }
        .ssn-user-avatar {
          width: 30px; height: 30px; border-radius: 50%;
          display: flex; align-items: center; justify-content: center;
          color: white; font-weight: 800; font-size: 0.82rem; flex-shrink: 0;
          letter-spacing: -0.02em;
        }
        .ssn-user-name {
          font-size: 0.82rem; font-weight: 700; color: var(--navy);
          max-width: 90px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
        }
        .ssn-chevron { transition: transform 0.2s var(--ease); color: var(--text-muted); }
        .ssn-chevron.open { transform: rotate(180deg); }

        /* Dropdown */
        .ssn-dropdown {
          position: absolute; right: 0; top: calc(100% + 10px);
          background: white; border-radius: 14px; min-width: 210px;
          box-shadow: var(--shadow-lg); border: 1px solid var(--border);
          overflow: hidden; z-index: 9999;
          animation: dropIn 0.18s var(--ease);
        }
        @keyframes dropIn {
          from { opacity: 0; transform: translateY(-6px) scale(0.98); }
          to   { opacity: 1; transform: translateY(0) scale(1); }
        }
        .ssn-dropdown-header {
          padding: 14px 16px 12px;
          border-bottom: 1px solid var(--border);
        }
        .ssn-dropdown-role {
          display: inline-flex; align-items: center; gap: 5px;
          padding: 2px 8px; border-radius: 999px;
          font-size: 0.68rem; font-weight: 700; letter-spacing: 0.06em;
          text-transform: uppercase; margin-bottom: 6px; color: white;
        }
        .ssn-dropdown-name { margin: 0; font-size: 0.9rem; font-weight: 700; color: var(--navy); }
        .ssn-dropdown-user { margin: 2px 0 0; font-size: 0.75rem; color: var(--text-muted); }
        .ssn-dropdown-item {
          display: flex; align-items: center; gap: 10px;
          padding: 11px 16px; font-size: 0.85rem; font-weight: 600;
          color: #374151; text-decoration: none; background: none;
          border: none; cursor: pointer; width: 100%; text-align: left;
          transition: background 0.13s; font-family: var(--font);
        }
        .ssn-dropdown-item:hover { background: var(--off-white); }
        .ssn-dropdown-item.danger { color: #DC2626; }
        .ssn-dropdown-item.danger:hover { background: #FEF2F2; }
        .ssn-dropdown-divider { height: 1px; background: var(--border); margin: 0; }

        /* ── Mobile ── */
        .ssn-hamburger {
          display: none; background: none; border: none;
          color: var(--navy); cursor: pointer; padding: 6px;
          border-radius: 6px; transition: background 0.15s;
        }
        .ssn-hamburger:hover { background: var(--off-white); }

        .ssn-mobile-drawer {
          display: none;
          background: white; border-top: 1px solid var(--border);
          padding: 16px 24px 24px;
          animation: slideDown 0.22s var(--ease);
        }
        @keyframes slideDown {
          from { opacity: 0; transform: translateY(-8px); }
          to   { opacity: 1; transform: translateY(0); }
        }
        .ssn-mobile-links { display: flex; flex-direction: column; }
        .ssn-mobile-link {
          padding: 11px 0; font-size: 0.92rem; font-weight: 600;
          color: var(--text-muted); text-decoration: none;
          border-bottom: 1px solid var(--border);
          transition: color 0.15s;
        }
        .ssn-mobile-link:last-child { border-bottom: none; }
        .ssn-mobile-link:hover, .ssn-mobile-link.active { color: var(--navy); }
        .ssn-mobile-link.active { font-weight: 700; }
        .ssn-mobile-actions {
          display: flex; gap: 8px; margin-top: 16px; padding-top: 16px;
          border-top: 1px solid var(--border); flex-wrap: wrap;
        }
        .ssn-mobile-user {
          display: flex; align-items: center; gap: 12px;
          padding: 12px 0 16px; border-top: 1px solid var(--border); margin-top: 8px;
        }
        .ssn-mobile-avatar {
          width: 42px; height: 42px; border-radius: 50%;
          display: flex; align-items: center; justify-content: center;
          color: white; font-weight: 800; font-size: 1rem;
        }
        .ssn-mobile-meta { flex: 1; }
        .ssn-mobile-meta-name { font-size: 0.9rem; font-weight: 700; color: var(--navy); margin: 0; }
        .ssn-mobile-meta-role { font-size: 0.75rem; font-weight: 600; margin: 2px 0 0; text-transform: capitalize; }
        .ssn-mobile-btns { display: flex; gap: 8px; }
        .ssn-mobile-dash-btn {
          flex: 1; padding: 10px 0; border-radius: 8px;
          color: white; font-weight: 700; text-align: center;
          text-decoration: none; font-size: 0.82rem; font-family: var(--font);
        }
        .ssn-mobile-logout-btn {
          flex: 1; padding: 10px 0; border-radius: 8px;
          background: #FEF2F2; color: #DC2626;
          border: 1.5px solid #FECACA;
          cursor: pointer; font-weight: 700; font-size: 0.82rem;
          font-family: var(--font);
        }

        @media (max-width: 1024px) {
          .ssn-links, .ssn-actions { display: none !important; }
          .ssn-hamburger { display: flex !important; align-items: center; }
          .ssn-mobile-drawer { display: block !important; }
        }
        @media (max-width: 480px) {
          .ssn-topbar-left { display: none; }
          .ssn-nav-inner { padding: 0 16px; }
          .ssn-logo-name { font-size: 0.82rem; }
          .ssn-logo-sub  { font-size: 0.65rem; }
        }
      `}</style>

      <div className="ssn-navbar">
        {/* ── Top bar ── */}
        <div className="ssn-topbar">
          <div className="ssn-topbar-inner">
            <div className="ssn-topbar-left">
              <span className="ssn-topbar-item">
                <FaPhone size={10} />
                {siteSettings?.phone || '+977-XXX-XXXXXXX'}
              </span>
              <span className="ssn-topbar-item">
                <FaEnvelope size={10} />
                {siteSettings?.email || 'info@ssnebs.edu.np'}
              </span>
            </div>
            <div className="ssn-topbar-right">
              <span className="ssn-topbar-label">Follow</span>
              <a href={siteSettings?.facebook || '#'} target="_blank" rel="noopener noreferrer" className="ssn-social-link ssn-social-fb">
                <FaFacebook size={13} />
              </a>
              <a href={siteSettings?.tiktok || '#'} target="_blank" rel="noopener noreferrer" className="ssn-social-link ssn-social-tik">
                <FaTiktok size={13} />
              </a>
            </div>
          </div>
        </div>

        {/* ── Main nav ── */}
        <nav className={`ssn-nav${scrolled ? ' scrolled' : ''}`}>
          <div className="ssn-nav-inner">

            {/* Logo */}
            <Link to="/" className="ssn-logo">
              <img src={logo} alt="SSNEBS" className="ssn-logo-img" />
              <div className="ssn-logo-text">
                <span className="ssn-logo-name">Siddhartha Sishu Niketan</span>
                <span className="ssn-logo-sub">English Boarding School</span>
              </div>
            </Link>

            {/* Desktop links */}
            <div className="ssn-links">
              {navLinks.map(({ path, label }) => (
                <Link
                  key={path} to={path}
                  className={`ssn-link${location.pathname === path ? ' active' : ''}`}
                >
                  {label}
                </Link>
              ))}
            </div>

            {/* Desktop actions */}
            <div className="ssn-actions">
              <button className="ssn-lang-btn" onClick={toggleLanguage}>
                {lang === 'en' ? 'NP' : 'EN'}
              </button>

              {!currentUser && (
                <Link to="/login" className="ssn-login-btn">
                  <FaSignInAlt size={13} /> Login
                </Link>
              )}

              {currentUser && (
                <div className="ssn-user-wrap" ref={userMenuRef}>
                  <button className="ssn-user-btn" onClick={() => setUserMenuOpen(o => !o)}>
                    <div className="ssn-user-avatar" style={{ background: roleColor }}>
                      {(currentUser.name || 'U').charAt(0).toUpperCase()}
                    </div>
                    <span className="ssn-user-name">{currentUser.name?.split(' ')[0]}</span>
                    <FaChevronDown size={9} className={`ssn-chevron${userMenuOpen ? ' open' : ''}`} />
                  </button>

                  {userMenuOpen && (
                    <div className="ssn-dropdown">
                      <div className="ssn-dropdown-header">
                        <span className="ssn-dropdown-role" style={{ background: roleColor }}>
                          {ROLE_ICON[currentUser.role]} {currentUser.role}
                        </span>
                        <p className="ssn-dropdown-name">{currentUser.name}</p>
                        <p className="ssn-dropdown-user">@{currentUser.username}</p>
                      </div>
                      <Link
                        to={ROLE_DASHBOARD[currentUser.role]}
                        onClick={() => setUserMenuOpen(false)}
                        className="ssn-dropdown-item"
                      >
                        <FaUserCircle style={{ color: roleColor }} size={14} />
                        My Dashboard
                      </Link>
                      <div className="ssn-dropdown-divider" />
                      <button onClick={handleLogout} className="ssn-dropdown-item danger">
                        <FaSignOutAlt size={14} /> Sign Out
                      </button>
                    </div>
                  )}
                </div>
              )}

              <Link to="/admissions" className="ssn-apply-btn">
                {t('applyNow')}
              </Link>
            </div>

            {/* Hamburger */}
            <button className="ssn-hamburger" onClick={() => setMobileOpen(o => !o)}>
              {mobileOpen ? <FaTimes size={20} /> : <FaBars size={20} />}
            </button>
          </div>

          {/* Mobile drawer */}
          {mobileOpen && (
            <div className="ssn-mobile-drawer">
              <div className="ssn-mobile-links">
                {navLinks.map(({ path, label }) => (
                  <Link
                    key={path} to={path}
                    className={`ssn-mobile-link${location.pathname === path ? ' active' : ''}`}
                  >
                    {label}
                  </Link>
                ))}
              </div>

              {currentUser ? (
                <>
                  <div className="ssn-mobile-user">
                    <div className="ssn-mobile-avatar" style={{ background: roleColor }}>
                      {(currentUser.name || 'U').charAt(0).toUpperCase()}
                    </div>
                    <div className="ssn-mobile-meta">
                      <p className="ssn-mobile-meta-name">{currentUser.name}</p>
                      <p className="ssn-mobile-meta-role" style={{ color: roleColor }}>{ROLE_LABEL[currentUser.role]}</p>
                    </div>
                  </div>
                  <div className="ssn-mobile-btns">
                    <Link to={ROLE_DASHBOARD[currentUser.role]} onClick={() => setMobileOpen(false)}
                      className="ssn-mobile-dash-btn" style={{ background: roleColor }}>
                      Dashboard
                    </Link>
                    <button onClick={() => { handleLogout(); setMobileOpen(false); }} className="ssn-mobile-logout-btn">
                      Sign Out
                    </button>
                  </div>
                </>
              ) : (
                <div className="ssn-mobile-actions">
                  <button onClick={toggleLanguage} className="ssn-lang-btn">
                    {lang === 'en' ? 'NP' : 'EN'}
                  </button>
                  <Link to="/login" className="ssn-login-btn" style={{ flex: 1, justifyContent: 'center' }}>
                    <FaSignInAlt size={13} /> Login
                  </Link>
                  <Link to="/admissions" className="ssn-apply-btn" style={{ flex: 2, justifyContent: 'center' }}>
                    {t('applyNow')}
                  </Link>
                </div>
              )}
            </div>
          )}
        </nav>
      </div>
      <div className="ssn-navbar-spacer" />
    </>
  );
};

export default Navbar;