...
import { defaultNews, defaultAchievements, defaultTestimonials, defaultPageContent } from '../data/defaultData';
import HeroSlider from '../components/HeroSlider';
import StatsCounter from '../components/StatsCounter';
import NoticeTicker from '../components/NoticeTicker';
import TestimonialCarousel from '../components/TestimonialCarousel';
import { defaultSiteSettings } from '../data/defaultData';
import { FaChild, FaBook, FaSchool, FaGraduationCap, FaAtom, FaBriefcase, FaArrowRight, FaFacebook, FaYoutube, FaInstagram, FaTiktok } from 'react-icons/fa';
...
const Home = () => {
  ...
  const principal = pageContent?.principal || defaultPageContent.principal;
            ...
            <div className="scroll-animate" style={{ textAlign: 'center' }}>
              {principal.image ? (
                ...
                <div style={{
                  ...
                  margin: '0 auto', boxShadow: '0 10px 40px rgba(26,58,107,0.3)',
                  fontSize: '5rem', color: 'white',
                  fontFamily: 'var(--font-display)', fontWeight: 900
                }}>
                  {(principal.name || 'P').charAt(0)}
                </div>
              )}
              <h3 style={{ marginTop: '20px', color: 'var(--color-secondary)' }}>{principal.name}</h3>
              <p style={{ color: 'var(--color-primary)', fontWeight: 600 }}>{principal.title}</p>
              {principal.qualifications && (
                <p style={{ color: 'var(--color-muted)', fontSize: '0.9rem' }}>{principal.qualifications}</p>
              )}
            </div>
            <div className="scroll-animate">
              <h2 style={{ fontSize: '2rem', marginBottom: '20px', color: 'var(--color-secondary)' }}>{t('principalMessage')}</h2>
              <div style={{ fontSize: '3rem', color: 'var(--color-accent)', lineHeight: 0, marginBottom: '20px' }}>❝</div>
              <p style={{ fontSize: '1.1rem', lineHeight: 1.8, color: 'var(--color-dark)', fontStyle: 'italic' }}>
                {principal.quote}
              </p>
              <div style={{ fontSize: '3rem', color: 'var(--color-accent)', lineHeight: 0, marginTop: '20px', textAlign: 'right' }}>❞</div>
            </div>
          </div>
        </div>
      </section>
